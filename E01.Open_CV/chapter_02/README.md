...
### 목표
마우스 이벤트와 상호작용을 통해 다음을 구현합니다:
1. 마우스 이벤트 감지 및 처리하기
2. 실시간으로 이미지 위에 그림 그리기
3. 키보드 입력으로 붓 크기 조절하기
4. 드래그를 통해 선 그리기

### 학습 포인트
- 마우스 콜백 함수 구현 (`setMouseCallback`)
- 마우스 이벤트 종류 이해 (클릭, 이동, 드래그)
- 도형 그리기 (`circle` 함수)
- 키보드 입력 처리 (`waitKey`)
- 전역 변수 활용
...
### 출력
- 마우스로 그린 파란색과 빨간색 원/선이 표시된 이미지
- 프로그램 종료 시 `soccer_painted.jpg`로 저장
